...
module Displayable
  ...
  def no_saved_games_message
    "Woops! You haven't saved any games. Type #{stylize '/back', BOLD} to go to the start screen"
  end
  ...
  def load_a_game_screen(files)
    return no_saved_games_message if file.empty?
  ...
  def hide_secret_word(secret_word, remaining_letters)
    ...
    mystery-word = secret_word.split('').join(' ')
    <<~HEREDOC
  ...
  def game_over_message(game_won, secret_word)
    if game_won
      ...
      puts "#{colorize LOSE_MESSAGE, COLOR[:fg][:red]} The word was \"#{colorize secret_word, COLORS[:fg][:green]}\""
    end
