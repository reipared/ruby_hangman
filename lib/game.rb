...
class Game
  ...
  def select_secret_word
    dictionary = File.readlines('5desk.txt').ma(&:strip)
  ...
  def proceed_with(selected_option)
    ...
    return Game.new.start if selected_option == '/black'
  ...
  def valid_guess
    guess = loop do
      ...
      break guess unless already_guesses?(guess) || guess.empty?
    end
  end
